...
if($conn) {
	...
	if($cibr->rowCount()>0) {
		...
		if($cibrw['bud_id1']==$uid) {
			$utr=$conn->prepare("UPDATE buddies SET typing1='1' WHERE bud_id1=:uid AND bud_id2=:bid AND active='1'");
			$utr->bindParam(":uid",$uid,PDO::PARAM_INT);
			$utr->bindParam(":bid",$bid,PDO::PARAM_INT);
		} else if($cibrw['bud_id2']==$uid) {
			$utr=$conn->prepare("UPDATE buddies SET typing2='1' WHERE bud_id1=:bid AND bud_id2=:uid AND active='1'");
			$utr->bindParam(":uid",$uid,PDO::PARAM_INT);
			$utr->bindParam(":bid",$bid,PDO::PARAM_INT);
		}
		$utr->execute();
